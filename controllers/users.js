...
export const updateProfile = async (req, res) => {
  try {
    const currentUser = await User.findByIdAndUpdate(req.user._id, req.body, { new: true });
    if (!currentUser) {
      throw new mongoose.Error.DocumentNotFoundError();
    }
...
export const updateAvatar = async (req, res) => {
  try {
    const currentUser = await User.findByIdAndUpdate(req.user._id, req.body.avatar, { new: true });
    if (!currentUser) {
      throw new mongoose.Error.DocumentNotFoundError();
    }
    return res.status(StatusCodes.OK).send(currentUser.avatar);
  } catch (error) {
    if (error instanceof mongoose.Error.ValidationError) {
      return res.status(StatusCodes.BAD_REQUEST).send({ message: 'Ошибка валидации', error: error.message });
    }
